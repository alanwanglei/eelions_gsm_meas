...
void proc_meas_ber(int txpwr)
{
	...
	// get BER
	DO_CMD("chans -a -l");
	str = strstr(resbuf, "TCH/F");
	if(str == NULL) {
		printf("Error: Can't get channel informations: txpower=%d\n", 80-txpwr);
		return;
	}
	str = strstr(str, "LinkEstablished false");
	if(str == NULL) {
		printf("Error: Can't get channel Link informations: txpower=%d\n", 80-txpwr);
		return;
	}
	str = str + strlen("LinkEstablished false") + 1;
	ber = 0.0;
	sscanf(str, "%*f %*f %*f %*f %*f %f", &ber);
	printf("BER informations: PWR=%3d, BER=%2.4f%%\n", 80-txpwr, ber);
